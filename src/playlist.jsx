...
export default function Playlist(){
  ...
  ({
    ...
    useLayoutEffect(()=>{
    axios.get('http://localhost:3000.com/getvideos')
    .then((res)=>{
  ...
  function addVideo(){
    if(videoinput && videoinput.length){
      console.log("sent")
       axios.post('http://localhost:3000.com/addvideo',{data:{link:videoinput}})
       .then((res)=>{
          setVideoinput("")
  ...
  function deleteVideo(e){
    ...
    axios.delete('http://localhost:3000.com/deletevideo',{data:{id:id}})
    .then((res)=>{
        if(res.data.message!="error"){
            setVideos((oldvideos)=>{
